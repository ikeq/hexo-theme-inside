...
/**
 *  This is a non-standard and partial implementation, therefore can not be used else where
 *
 * @param {*} schema
 * @param {*} data
 * @param {*} payload
 */
exports.parseSchema = function(schema, data, payload = {}) {
  ...
  function parse(sub, subv, paths) {
    ...
    // object can hold a `required` key, To ensure that the correct hint can be dug,
    ...
    if (sub.enum) {
      ...
      if (!Object.keys(safeSubv).length) safeSubv = undefined

      // Dependencies
      if (safeSubv && sub.dependencies) {
        for (const depKey in sub.dependencies) {
          const dep = sub.dependencies[depKey]
          // Schema dependencies
          if (dep.properties) {
            const result = parse({ type: 'object', ...dep}, safeSubv, paths + '/dependencies/' + depKey + '/')
            if (!result.pass) return ret
          }
          // Property dependencies
          else {
            for (const depTargetKey of dep) {
              if (!safeSubv[depTargetKey]) return ret
            }
          }
        }
      }
