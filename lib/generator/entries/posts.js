...
module.exports = function ({ theme, site, locals, helpers }) {
  ...
  return flattenDeep([
    posts.map((post, i) => {
      ...
      // comments
      if (!theme.comments.disqus || !post.comments) delete post.comments;
      ...
      if (copyright) {
        ...
        if (cr.custom) cr = { global: cr.global, custom: cr.custom };
          ...
          if (cr.link) cr.link = `<a href="${post.permalink}" title="${post.title}">${post.permalink}</a>`;
          else delete cr.link;
