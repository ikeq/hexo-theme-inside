  ...
  hexo.on('generateBefore', function () {
    ...
    if (result.sns) {
      ...
      result.sns = sns.reduce((ret, i) => {
        if (i.icon) {
          if (i.icon === 'email' && i.url === null) {
            result.sns.email = 'mailto:' + (result.sns.email || email);
          }
          else if (i.icon === 'feed' && i.url === null) {
            result.sns.feed = result.sns.feed || feed;
          }
          if (i.url) {
...

    {
      ...
      if (Array.isArray(result.plugins)) {
        result.plugins = {
          dir: [],
          definitions: result.plugins,
        };
      }

      const entries = [
        // plugins comes first to ensure that their libs is ready when executing dynamic code.
        ...result.plugins.definitions,
        ...feManifest.styles,
        ...feManifest.scripts,
      ];
      const pluginManifest = loadManifest(this, [path.join(__dirname, 'plugins')].concat(result.plugins.dir || []));
