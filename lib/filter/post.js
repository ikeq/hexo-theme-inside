...
module.exports = function (data) {
  ...
  const hasComments = !!(theme.comments || theme.plugins && theme.plugins.comments);
  const hasReward = !!theme.reward;
  const hasToc = !!theme.toc;
  const copyright = theme.copyright;
  const dateHelper = date.bind({ page: { lang: localeId(config.language, true) }, config })
  const uriReplacer = (s, p) => /^(\/\/|http|data\:image)/.test(s) ?
    s :
    (bs => theme.assets ? 
      bs.replace(/^\/*(.*[^\/])\/*$/, theme.assets.prefix + '/$1' + theme.assets.suffix) :
      bs
    )(config.post_asset_folder ? s.startsWith('/') ? s : `/${p}/${s}` : s)
