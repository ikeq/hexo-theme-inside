...
// cache
let hasComments, hasReward, hasToc, copyright, dateHelper, uriRplacer;
...
module.exports = function (data) {
  ...
  if (uriRplacer === undefined)
    uriRplacer = theme.assets ?
      (() => {
  ...
  if (!isPage && data.thumbnail) {
    ...
    data.thumbnail = uriRplacer(particals[0]);
  ...
  function processHtml(html, { link }) {
    ...
    $('img').each(function () {
      ...
      src && img$.attr('src', uriRplacer(src))
    });
