...
module.exports = function (data) {
  ...
  const { hasComments, hasReward, hasToc, copyright, dateHelper, uriReplacer } = theme.runtime;
  ...
  // avoid css class conflict
  data.content = removeHljsLang(data.content);
  data.excerpt = removeHljsLang(data.excerpt);
};

function removeHljsLang(html) {
  return html.replace(/(?<=figure class=\\?"highlight) \w+/g, '');
}
